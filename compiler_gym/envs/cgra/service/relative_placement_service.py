...
class RelativePlacementCGRASession(CGRASession):
    ...
    def get_initial_placement_nth_avail(self, dfg, seed, n):
        ...
        initial_II, _ = self.schedule.get_II(dfg)
        print("After initial placement (mode, first_avail), got II", initial_II)
        assert initial_II is not None #should be a valid schedule.
    ...
    def apply_action(self, action: Event) -> Tuple[bool, Optional[ActionSpace], bool]:
        try:
            ...
            print("Before swap, II is ", self.schedule.get_II(self.dfg), "iteration is ", self.iteration_number)
            ...
            print("II is ", self.schedule.get_II(self.dfg))
